...
document.getElementById('metaForm').addEventListener('submit', async function (e) {
    ...
    try {
        // Verificar si ya existe una meta del mismo tipo
        const responseCheck = await fetch(`meta/meta_usuario_obtener.php?usuario_id=${currentUser.id}`);
        const dataCheck = await responseCheck.json();

        if (dataCheck.success) {
            const metaExistente = dataCheck.data.find(m =>
                m.tipo === formData.tipo && m.id !== formData.id
            );

            if (metaExistente && !editingMeta) {
                CustomAlert.show('Ya tienes una meta de ' + formData.tipo + ' establecida. Edita la existente en lugar de crear una nueva.', 'warning');
                return;
            }
        }
        ...
        if (result.success) {
            ...
            if (result.message.includes('ya existe')) {
                CustomAlert.show('Ya tienes una meta de este tipo establecida', 'warning');
            } else {
